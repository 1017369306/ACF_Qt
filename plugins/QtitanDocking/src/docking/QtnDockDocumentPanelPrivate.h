...
public:
    ...
    //add by zhangyang 20230910 新增的函数，用于控制tabcontrol的bar的状态（tooltip、是否可关闭），发现控制不了（内部使用的是一个指针对象）
    void setFeatures(const DockWidgetPanel::DockPanelFeatures& features);
    void setTabTooltip(const QString& tooltip);

    //add by zhangyang 20230910 新增的重写函数，未使用到
//    void initialization(DockWidgetPanel* panel) override;
